...
def earth_to_body_frame(ii, jj, kk):
    # C^b_n
    R = [[C(kk) * C(jj), C(kk) * S(jj) * S(ii) - S(kk) * C(ii),
          C(kk) * S(jj) * C(ii) + S(kk) * S(ii)],
         [S(kk) * C(jj), S(kk) * S(jj) * S(ii) + C(kk) * C(ii),
          S(kk) * S(jj) * C(ii) - C(kk) * S(ii)],
...
class PhysicsSim():
    ...
    def reset(self):
        self.time = 0.0
        self.pose = np.array([0.0, 0.0, 10.0, 0.0, 0.0, 0.0]) if self.init_pose\
                                            is None else np.copy(self.init_pose)
        self.v = np.array([0.0, 0.0, 0.0]) if self.init_velocities is\
                                        None else np.copy(self.init_velocities)
        self.angular_v = np.array([0.0, 0.0, 0.0]) if self.init_angle_velocities\
                                is None else np.copy(self.init_angle_velocities)
    ...
    def get_linear_forces(self, thrusts):
        ...
        linear_forces = np.matmul(body_to_earth_frame(*list(self.pose[3:])),
                                  body_forces)
    ...
    def get_moments(self, thrusts):
        ...
        drag_moment =  self.C_d * 0.5 * self.rho * self.angular_v * \
                       np.absolute(self.angular_v) * self.areas * self.dims\
                       * self.dims
